...
function resolveVitestEnvironment() {
  ...
  try {
    return fileURLToPath(
      new URL('../../vitest-environment-untestutils/index.mjs', import.meta.url),
    );
  } catch {
    return 'untestutils';
  }
...
function warnIfE2ePluginMixed(config) {
  ...
  const hasE2e = plugins.some(
    (p) => p && typeof p === 'object' && 'name' in p && p.name === 'untestutils',
  );
...
async function startNuxtAndGetViteConfig(rootDir = process.cwd(), options = {}) {
  ...
  if (!nuxt.options._installedModules.find((i) => i?.meta?.name === 'untestutils'))
    throw new Error(
      'Failed to load `untestutils/module`. You may need to add it to your nuxt.config.',
    );
...
async function getVitestConfigFromNuxt(options, loadNuxtOptions = {}) {
  ...
  options.viteConfig.plugins = (options.viteConfig.plugins || []).filter(
    (p) => !p || !('name' in p) || !excludedPlugins.includes(p.name),
  );
  ...
  const h3Info =
    (projectH3Path ? getPackageInfoSync('h3', { paths: [projectH3Path] }) : void 0) ||
    getPackageInfoSync('h3', { paths: nitroPath ? [nitroPath] : options.nuxt.options.modulesDir });
        ...
        environmentOptions: {
          nuxtRuntimeConfig: applyEnv(deepCopy(options.nuxt.options.runtimeConfig), {
            ...
            env: await setupDotenv(
              defu$1(loadNuxtOptions.dotenv, {
                cwd: rootDir,
                fileName: '.env.test',
              }),
            ),
          }),
          nuxtRouteRules: defu$1(
            {},
...
async function defineVitestProject(config) {
  const resolvedConfig = await resolveConfig(
    defu$1({ test: { environment: resolveVitestEnvironment() } }, config),
  );
...
function defineVitestConfig(config = {}) {
  return defineViteConfig(async () => {
    ...
    const isUnitEnv =
      isUnitEnvironmentName(defaultEnvironment) || defaultEnvironment === unitEnvironment;
    if (!isUnitEnv) {
  ...
  if (resolvedConfig.test.browser?.enabled) {
    ...
    if (resolvedConfig.test.environment === 'untestutils') {
      resolvedConfig.test.setupFiles = Array.isArray(resolvedConfig.test.setupFiles)
        ...
        : [resolvedConfig.test.setupFiles].filter(Boolean);
      try {
