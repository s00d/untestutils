...
async function createFetchForH3V1() {
	const [{ createApp, toNodeListener }, { fetchNodeRequestHandler }] = await Promise.all([import("h3"), import("node-mock-http")]);
	...
	const h3Fetch = (async (input, _init) => {
		...
		if (url.startsWith("/")) return normalizeFetchResponse(await fetchNodeRequestHandler(nodeHandler, url, init));
...
function normalizeCookieHeaders(headers) {
	...
	for (const [name, header] of headers) if (name === "set-cookie") for (const cookie of normalizeCookieHeader(header)) outgoingHeaders.append("set-cookie", cookie);
...
var happy_dom_default = async function(_, { happyDom = {} }) {
...
var jsdom_default = async function(global, { jsdom = {} }) {
...
var vitest_default = {
	...
	async setup(global, environmentOptions) {
		...
		const { window: win, teardown } = await (environmentMap[environmentName] || environmentMap["happy-dom"])(global, defu(environmentOptions, {
			happyDom: { url },
			jsdom: { url }
		}));
		if (environmentOptions.nuxt?.mock?.intersectionObserver) win.IntersectionObserver ||= IntersectionObserver;
		...
		return { teardown() {
			...
			teardownWindow();
			if (vitestMajor >= 5) originals.forEach((descriptor, k) => Object.defineProperty(global, k, descriptor));
...
async function importVitestEnvironments() {
	return await import(
		/* @vite-ignore */
		["vitest", vitestMajor >= 5 || resolveModulePath("vitest/runtime", { try: true }) ? "runtime" : "environments"].join("/")
);
}
